...
from horilla.horilla_middlewares import sub_apps
...
@register.filter(name='has_app_perm')
def has_app_perm(user, app_name):
    """
    تحقق مما إذا كان المستخدم لديه اشتراك يسمح له باستخدام تطبيق معين.
    """
    app_permission = sub_apps.get(app_name, None)
    
    if hasattr(user, 'employee_get'):
        if app_permission:
            return user.employee_get.sub_has_perm(app_permission)
        else:return True
    return False