...
from subscriptions.models import UserSubscriptionModel, SubscriptionsModel
# create your model
...
class Employee(models.Model):
    ...
    def get_user_sub(self):
        user_sub = UserSubscriptionModel.objects.first()
        if not user_sub:
            sub = SubscriptionsModel.objects.create(title='Default')
            sub.save()
            user_sub = UserSubscriptionModel.objects.create(subscription=sub)
            user_sub.save()
        return user_sub
        
    @property
    def has_subscription(self):
        user_sub = self.get_user_sub
        if user_sub:
            return user_sub.is_active
        return False
    
    def sub_has_perm(self, perm):
        user_sub = self.get_user_sub
        if user_sub:
            if user_sub.is_active:
                lim = getattr(user_sub.subscription, perm, False)
                return lim
        return False

    def sub_has_perm_number(self, perm):
        lim = self.sub_has_perm(perm)
        if lim:
            parent = self.parent
            if parent:
                if perm == 'number_of_employees':
                    emp = Employee.objects.filter(parent=parent)
                    if emp.count() < lim:
                        return True
                elif perm == 'number_of_managers':
                    users = User.objects.filter(employee_get__parent=parent, is_superuser=True)
                    if users < lim:
                        return True
        return False
